...
export const FAQSection = ({ entries = [], isAdmin, userEmail }) => {
  const { t } = useTranslation();
  const [searchTerm, setSearchTerm] = useState('');
  const [showModal, setShowModal] = useState(false);
  const [formMode, setFormMode] = useState('create'); // 'create', 'edit', or 'answer'
  ...
  // For non-admin users, only display entries with status 1.
  ...
  // Open the DeleteModal instead of using a simple confirm
  const handleDelete = (entry) => {
    setEntryToDelete(entry);
    setShowDeleteModal(true);
  };

  // When deletion is confirmed, perform the delete via Inertia.
  const handleConfirmDelete = () => {
    if (entryToDelete) {
      Inertia.delete(route('help.entry.destroy', entryToDelete.id));
  ...
  return (
    <div  className="rounded-2xl bg-gradient-to-br from-black via-gray-900 to-zinc-900 text-white p-8 mb-10">
      <div className="flex justify-between items-center mb-6 ">
        <h2 className="text-3xl font-bold text-gray-800">{t('faq_section.faq_title')}</h2>
        <button
          onClick={handleAddClick}
          className="flex items-center space-x-2 text-blue-600 hover:underline"
        >
          <i className="fas fa-plus-circle"></i>
          <span>{t('faq_section.add_question_prompt')}</span>
        </button>
      </div>
      <input
        type="text"
        placeholder={t('faq_section.search_faqs')}
        value={searchTerm}
      ...
      <HelpForm
        ...
        userEmail={userEmail}
      />
      {/* Reusable Delete Modal for confirming deletion */}
      <DeleteModal
...
const FAQItem = ({ entry, isAdmin, onEdit, onAnswer, onDelete }) => {
  const { t } = useTranslation();
  ...
  return (
    <div className="border border-gray-200 rounded-lg overflow-hidden">
      ...
      >
        <span className="text-lg font-medium text-gray-700 flex items-center">
          ...
          {isAdmin && entry.status === 0 && (
            <i className="fas fa-eye-slash text-red-500 ml-2" title={t('faq_section.hidden')}></i>
          )}
        </span>
      ...
      <div className={`px-4 overflow-hidden transition-all duration-300 ${isOpen ? 'max-h-96 py-4 border-t border-gray-200' : 'max-h-0'}`}>
